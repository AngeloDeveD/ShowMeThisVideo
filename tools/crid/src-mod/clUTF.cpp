...
bool clUTF::LoadData(void *data)
{
	...
	for (unsigned int i = 0; i < info->valueCount; i++)
	{
		...
		for (unsigned int j = 0; j < info->elementCount; j++)
		{
			...
			unsigned int offset = bswap(*(unsigned int *)s);
			s += sizeof(offset);
			...
			case 0x11:
				...
				*p += sizeof(unsigned char);
				break;
			case 0x12:
				e->SetValueShort(bswap(*(short *)*p));
				*p += sizeof(short);
				break;
			case 0x13:
				e->SetValueUShort(bswap(*(unsigned short *)*p));
				*p += sizeof(unsigned short);
				break;
			case 0x14:
				e->SetValueInt(bswap(*(int *)*p));
				*p += sizeof(int);
				break;
			case 0x15:
				e->SetValueUInt(bswap(*(unsigned int *)*p));
				*p += sizeof(unsigned int);
				break;
			case 0x16:
				e->SetValueLongLong(bswap(*(long long *)*p));
				*p += sizeof(long long);
				break;
			case 0x17:
				e->SetValueULongLong(bswap(*(unsigned long long *)*p));
				*p += sizeof(unsigned long long);
				break;
			case 0x18:
				e->SetValueFloat(bswap(*(float *)*p));
				*p += sizeof(float);
				break;
			case 0x1A:
				e->SetValueString(&_string[bswap(*(unsigned int *)*p)]);
				*p += sizeof(unsigned int);
				break;
			case 0x1B:
				e->SetValueData(&_data[bswap(*(unsigned int *)*p)], (int)bswap(*(unsigned int *)(*p + 4)));
...
bool clUTF::SaveFileINI(FILE *fp, bool subUTF, int tab)
{
	...
	for (unsigned int i = 0, count = _pageCount; i < count; i++)
	{
		...
		for (clElement *e = _page[i].first; e; e = e->_next)
		{
			...
			switch (e->_type)
			{
			case clElement::TYPE_CHAR:
				fprintf(fp, "%d", e->_valueChar);
				break;
			case clElement::TYPE_UCHAR:
				fprintf(fp, "%u", e->_valueUChar);
				break;
			case clElement::TYPE_SHORT:
				fprintf(fp, "%d", e->_valueShort);
				break;
			case clElement::TYPE_USHORT:
				fprintf(fp, "%u", e->_valueUShort);
				break;
			case clElement::TYPE_INT:
				fprintf(fp, "%d", e->_valueInt);
				break;
			case clElement::TYPE_UINT:
				fprintf(fp, "%u", e->_valueUInt);
				break;
			case clElement::TYPE_LONGLONG:
				fprintf(fp, "%lld", e->_valueLongLong);
				break;
			case clElement::TYPE_ULONGLONG:
				fprintf(fp, "%llu", e->_valueULongLong);
				break;
			case clElement::TYPE_FLOAT:
				fprintf(fp, "%g", e->_valueFloat);
				break;
			case clElement::TYPE_STRING:
				fprintf(fp, "%s", e->_valueString);
				break;
			case clElement::TYPE_DATA:
				...
				{
					for (unsigned char *s = (unsigned char *)e->GetData(), *p = s + e->GetDataSize(); s < p; s++)
						fprintf(fp, "%02X ", *s);
				}
				break;
			}
...
clUTF::clElement *clUTF::Add(unsigned int pageIndex, const char *name, long long value)
{
	clElement *element = Add(&_page[pageIndex], name);
	if (element)
		element->SetValueLongLong(value);
	return element;
}
...
clUTF::clElement *clUTF::Add(unsigned int pageIndex, const char *name, void *data, unsigned int size)
{
	clElement *element = Add(&_page[pageIndex], name);
	if (element)
		element->SetValueData(data, size);
	return element;
}
