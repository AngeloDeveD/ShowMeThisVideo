...
char *FixFilename(char *fix_filename, int size, const char *filename)
{
	...
	for (int i = 0, len = strlen(filename); i < len && i < size - 3; i++, filename++)
	{
		...
		{
		case '*':
			*(WCHAR *)&fix_filename[i++] = bswap((WCHAR)'–');
			break;
		case '|':
			*(WCHAR *)&fix_filename[i++] = bswap((WCHAR)'b');
			break;
		case '\\':
			*(WCHAR *)&fix_filename[i++] = bswap((WCHAR)'');
			break;
		case ':':
			*(WCHAR *)&fix_filename[i++] = bswap((WCHAR)'F');
			break;
		case '"':
			*(WCHAR *)&fix_filename[i++] = bswap((WCHAR)'h');
			break;
		case '<':
			*(WCHAR *)&fix_filename[i++] = bswap((WCHAR)'ƒ');
			break;
		case '>':
			*(WCHAR *)&fix_filename[i++] = bswap((WCHAR)'„');
			break;
					...
					case 0x40534641:
						if (!fpAudio && is_demux_audio)
						{
							...
							if (is_convert_adx && strcmp(GetExtension(ext, _countof(ext), filename), "wav") != 0)
								strcat_s(filename, _countof(filename), ".wav");
